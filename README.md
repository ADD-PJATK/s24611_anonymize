...
**1. Markdown file (`.md`)**
```bash
python anonymize.py --mapping examples/mapping.json --input examples/note.md --output out/note.anon.md -v
```
...
3. **Scan Policy**: For each `find` string, the script scans the text left-to-right and replaces all occurrences with the `replace` token. Since the search is applied sequentially across entries, if there is an overlap (e.g., `"ab"` vs `"abc"`), you should place the longer match first in your `find` array (e.g., `["abc", "ab"]`). This ensures the longer string gets replaced before the shorter substring can match.
